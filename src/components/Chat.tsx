...
const Chat: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    ...
    {
        role: 'user',
        content: "Can you give me an example of a creative prompt?"
    },
    {
        role: 'model',
        content: "Of course! How about this for an idea:\n\n'A bioluminescent jellyfish floating gracefully through a cosmic nebula, its tentacles trailing stardust. The scene is illuminated by the soft glow of a distant, purple planet.'\n\nFor the 'Prompt Enhancer', you could try the **Fantasy Art** style with **Cinematic** lighting and a **Mysterious** mood to really make it pop!"
    }
  ]);
        ...
        {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-lg px-4 py-2 rounded-2xl animate-messagePopIn ${msg.role === 'user' ? 'bg-purple-600 text-white rounded-br-none' : 'bg-slate-700 text-gray-200 rounded-bl-none'}`}>
                  <p className="whitespace-pre-wrap text-md" dangerouslySetInnerHTML={{ __html: parseMarkdown(msg.content) }}></p>
              </div>
            </div>
        ))}
        {isLoading && (
